<footer class="container" style="bottom: 0;
    padding: 20px calc(24px / 2);
    position: absolute;
    right: 0;
    color: #74788d;
    left: 0px;
    height: 60px;
    background-color: #f2f2f5;">
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-6">
                <script>
                    document.write(new Date().getFullYear())
                </script> © ePawn Shop Philippines.
            </div>
            <div class="col-sm-6">
                <div class="text-sm-end d-none d-sm-block">
                    Design & Develop by Group 5
                </div>
            </div>
        </div>
    </div>
</footer>


<!-- JAVASCRIPT -->
<script src="<?php echo base_url('assets') ?>/libs/jquery/jquery.min.js"></script>
<script src="<?php echo base_url('assets') ?>/libs/bootstrap/js/bootstrap.bundle.min.js"></script>
<script src="<?php echo base_url('assets') ?>/libs/metismenu/metisMenu.min.js"></script>
<script src="<?php echo base_url('assets') ?>/libs/simplebar/simplebar.min.js"></script>
<script src="<?php echo base_url('assets') ?>/libs/node-waves/waves.min.js"></script>


<!-- Ion Range Slider-->
<script src="<?php echo base_url('assets') ?>/libs/ion-rangeslider/js/ion.rangeSlider.min.js"></script>

<!-- init js -->
<script src="<?php echo base_url('assets') ?>/js/pages/product-filter-range.init.js"></script>

<!-- Plugins js-->
<script src="<?php echo base_url('assets') ?>/libs/jquery-countdown/jquery.countdown.min.js"></script>

<!-- Countdown js -->
<script src="<?php echo base_url('assets') ?>/js/pages/coming-soon.init.js"></script>

<!-- date -->
<script src="<?php echo base_url('assets') ?>/libs/select2/js/select2.min.js"></script>
<script src="<?php echo base_url('assets') ?>/libs/bootstrap-datepicker/js/bootstrap-datepicker.min.js"></script>
<script src="<?php echo base_url('assets') ?>/libs/spectrum-colorpicker2/spectrum.min.js"></script>

<!-- form advanced init include select and date-->
<script src="<?php echo base_url('assets') ?>/js/pages/form-advanced.init.js"></script>

<!-- parsley -->
<script src="<?php echo base_url('assets') ?>/libs/parsleyjs/parsley.min.js"></script>
<!-- custom js -->
<script src="<?php echo base_url('assets') ?>/js/common.js"></script>
<script src="<?php echo base_url('assets') ?>/js/item/item.js"></script>
<script src="<?php echo base_url('assets') ?>/js/client/profile.js"></script>

<script src="<?php echo base_url('assets') ?>/js/app.js"></script>


</body>

</html>